...
def main():
    ...
    for line in asm:
        ...
        # = = = = ORI = = = = = = = = = (I)
            ...
            line = line.split(
                ",")  # split the 1 string 'line' into a string array of many strings, broken at the comma.
            rs = format(int(line[0]), '05b')  # make element 0 in the set, 'line' an int of 5 bits. (rt)
            imm = format(int(line[1]), '016b') if (int(line[1]) >= 0) else format(65536 + int(line[1]), '016b')
            rt = format(int(line[2]), '05b')  # make element 1 in the set, 'line' an int of 5 bits. (rs)
